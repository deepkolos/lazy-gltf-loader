...
class GLTFParser {
	...
	loadScene( sceneIndex ) {
		...
		for ( let i = 0, il = nodeIds.length; i < il; i ++ ) {
			...
			pending.push( buildNodeHierachy$1( nodeIds[ i ], scene, json, parser ) );
...
function buildNodeHierachy$1( nodeId, parentObject, json, parser ) {
	...
	return parser.getDependency( 'node', nodeId ).then( function ( node ) {
		...
		if ( nodeDef.children ) {
			...
			for ( let i = 0, il = children.length; i < il; i ++ ) {
				...
				pending.push( buildNodeHierachy$1( child, node, json, parser ) );
...
class LazyGLTFParser extends GLTFParser {
  constructor(json, options) {
    super(json, options);
...
export { LazyGLTFLoader, LazyGLTFParser };
